        ...
        return Commands.literal(name)
                ...
                .executes(context -> {
                    ...
                    if (anchorPlayer == null) {
                        context.getSource().sendFailure(Component.translatable("command.minezero.restore_anchor_offline", anchorPlayerUUID.toString()));
                        LOGGER.warn("Attempted to restore checkpoint for anchor {}, but player is not online.", anchorPlayerUUID);
                        return 0;
                    }

                    context.getSource().sendSuccess(() -> Component.translatable("command.minezero.restore_starting", anchorPlayer.getName()), true);
                    ...
                    level.getServer().execute(() -> {
                        ...
                        level.getServer().getPlayerList().getPlayers().forEach(p -> {
                            p.displayClientMessage(Component.translatable("message.minezero.restore_broadcast"), false);
                        });
