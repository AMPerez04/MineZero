...
- Config: hard rename — NeoForge regenerates the file with the new `[sounds] restoreSound` key at its default. The legacy values `CLASSIC`/`ALTERNATE` are still accepted if set manually.
