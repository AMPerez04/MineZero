...
- Gamerules (constants in `ModGameRules.java` now mirror the registered keys):
    ...
    - `enableRandomAnchor` → `randomAnchorEnabled`
    - `enableAllPlayersRBD` → `anyPlayerDeathTriggersRestore`
    - `autoCheckpointEnabled`, `artifactFluteEnabled`, and `checkpointOnSleep` are unchanged.
