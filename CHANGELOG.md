...
- Gamerules (constants in `ModGameRules.java` now mirror the registered keys; on Fabric use them as `/gamerule minezero:<key>`):
...
- Config (`config/minezero.json`, Cloth Config): `deathChime` (`CLASSIC`/`ALTERNATE`) → `restoreSound` (`death_chime`/`alt_death_chime`, matching the mod's sound IDs). The Mod Menu config screen category is now "Sound Settings".
...
- Gamerules: hard rename, no aliases — gamerules are independent registrations, so an "alias" would be a second rule whose value silently diverges from the real one. Existing worlds keep the old entries in level.dat harmlessly; the renamed rules start at their defaults, so re-apply any non-default values with `/gamerule` after updating.
- Config: hard rename — the config regenerates with the new `restoreSound` field at its default. The legacy values `CLASSIC`/`ALTERNATE` are still accepted if set manually.
